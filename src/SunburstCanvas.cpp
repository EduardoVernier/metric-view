...
template<typename T>
bool inBounds(const T &value, const T &low, const T &high) { return value >= low && high >= value; }
                            ...
                            for (auto e : p->entityVector) {
                                for (auto ep : entityData->entities) {
                                    if (e.getName() == ep->getName()) {
                                        if ((std::find(entityData->selected.begin(), entityData->selected.end(), ep)) !=
                                            entityData->selected.end())
                                            entityData->selected.erase(
                                                    std::find(entityData->selected.begin(), entityData->selected.end(),
                                                              ep));
                                        else
