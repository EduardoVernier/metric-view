    ...
    for (vector<BaseEntity *>::iterator b = entityData->sortedBaseEntities.begin();
        ...
        if ((*b)->isEntity()) {
            if ((*b)->getCoord(3) * yRatio < drag[1]) continue; // b too high
                ...
                if (click) {
                    if ((std::find(entityData->selected.begin(), entityData->selected.end(), (Entity *) *b)) !=
                        entityData->selected.end())
                        entityData->selected.erase(
                                std::find(entityData->selected.begin(), entityData->selected.end(), (Entity *) *b));
                    else
...
void TreemapCanvas::drawCanvas(unsigned Rt, double animationStep) {
    ...
    for (vector<BaseEntity *>::iterator it = entityData->sortedBaseEntities.begin();
         it != entityData->sortedBaseEntities.end(); ++it) {
        if ((*it)->isPackage()) {
